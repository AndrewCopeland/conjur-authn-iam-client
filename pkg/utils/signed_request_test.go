...
func TestGetAmzDate(t *testing.T) {
	...
	time, _ := time.Parse(layoutISO, date)
	actual := utils.GetAmzDate(time)
	assertStringEquals(t, actual, expected)
}

func TestSHA256Hash(t *testing.T) {
	expected := "b94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9"
	actual := utils.SHA256Hash("hello world")
	assertStringEquals(t, actual, expected)
}

func TestCreateStringToSign(t *testing.T) {
	actual := utils.CreateStringToSign(expectedDateAsString, expectedAmzDateAsString, expectedCanonicalRequest)
...
func TestCreateCanonicalRequest(t *testing.T) {
	expected := "20060102T220405Z"
	time, _ := time.Parse(layoutISO, date)
	amzDate := utils.GetAmzDate(time)
	assertStringEquals(t, amzDate, expected)

	actual := utils.CreateCanonicalRequest(amzDate, sessionToken, signedHeaders, payloadHash)
...
func TestGetAuthenticationRequest(t *testing.T) {
	...
	actual, err := utils.GetAuthenticationRequest("accessKey", "secretKey", "sessionToken", time)
	if err != nil {
